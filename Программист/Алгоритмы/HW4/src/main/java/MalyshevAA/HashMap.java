...
//    }
    ...
    class HashMapIterator implements Iterator<HashMap.Entity> {
        ...
        public Iterator iterator() {
            Iterator<Entity> HashMapIterator = new Iterator<>() {
                private int indexBucket = 0;
                private Bucket.Node curNode = null;

                @Override
                public boolean hasNext() {
                    while (indexBucket < buckets.length) {
                        if (buckets[indexBucket] == null) {
                            indexBucket++;
                            continue;
                        }
                        if (curNode == null) {
                            curNode = buckets[indexBucket].head;
                            return true;
                        }
